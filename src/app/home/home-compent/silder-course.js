...
import CourseCardTooltip from "@/app/courses/courses-compent/popover-courses/course-card";

const SilderCourse = (props) => {
  const { limit, col = { xs: 12, sm: 12, md: 12, lg: 12, xl: 12, xxl: 12 } } =
    props;
    ...
    <>
      ...
      >
        {NewCoursesLimit.map((items) => (
          <SwiperSlide key={items.id} className="relative hover:z-[99]">
            
              <div className="8888888">
                <CoursesListModule
                  items={items}
                  limit={limit}
                  MAX_LENGTH="30"
