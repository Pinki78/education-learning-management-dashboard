...
console.log(`/courses/${slugify(items.title)}`);
 ...
 trigger="hover"
  placement="left"
  content={<CourseCardTooltip  memberNames={memberNames} items={items} MAX_LENGTH_TITLE={MAX_LENGTH_TITLE}  />}
  theme={{
    base: "z-[9999]"
  }}
          ...
          >
            ...
            <div className={`flex items-center justify-left`}>
              <h5 className={`bx-badge-views font-(family-name:--font-roboto)`}>
                <Link  href={`/courses/category/${slugify(items.CoursesCategoryName)}`}>{items.CoursesCategoryName}</Link>
              </h5>
            ...
            <div className={`bx-fcrse-view-more`}>
              <Link  href={`/courses/${slugify(items.title)}`} className={`bx-view-more`}>
                View More
              </Link>
            </div>
          </div>
