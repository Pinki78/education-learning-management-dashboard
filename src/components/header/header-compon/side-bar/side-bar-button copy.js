...
const SideBarButton = () => {
  ...
  const { activeItemId, mobileOpen }= useSelector((state) => state.headerMenuReducer); // assuming your slice has 'open'

 const handleClick = () => {
    if (window.innerWidth < 560) {
      // ✅ mobile — toggle mobileOpen
      dispatch(setMobileOpen(!mobileOpen));
    } else {
      // ✅ desktop — toggle activeItemId as before
